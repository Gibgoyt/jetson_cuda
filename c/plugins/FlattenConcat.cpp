	...
	public:
	...
	#endif
		{
			...
			for (size_t i = 0; i < mFlattenedInputSize.size(); ++i) {
				...
				for (int batchIdx = 0; batchIdx < batchSize; ++batchIdx) {
					CHECK(cublasScopy(
					    mCublas,
					    mFlattenedInputSize[i],
					    input + batchIdx * mFlattenedInputSize[i],
					    1,
					    output + (batchIdx * mFlattenedOutputSize + inputOffset),
					    1
					));
				}
		...
		IPluginV2* clone() const NOEXCEPT override {
			return new FlattenConcat(
			    mFlattenedInputSize.data(),
			    mFlattenedInputSize.size(),
