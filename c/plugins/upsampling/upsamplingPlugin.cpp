...
int NearestNeighborUpsamplingPlugin::enqueue(
    int batchSize,
	...
	if (mDataType == DataType::kFLOAT)
		CHECK(
		    cudaResizeNearestNeighbor<float>(
		        (float*)inputs[0],
		        mNbInputChannels,
		        mInputHeight,
		        mInputWidth,
		...
		CHECK(
		    cudaResizeNearestNeighbor<__half>(
		        (__half*)inputs[0],
		        mNbInputChannels,
		        mInputHeight,
		        mInputWidth,
...
IPluginV2* NearestNeighborUpsamplingPluginCreator::createPlugin(
    const char* name,
