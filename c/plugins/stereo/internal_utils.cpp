...
size_t DimsUtils::getTensorSize(Dims dims) {
	...
	assert(
	    res == (size_t)std::accumulate(dims.d, dims.d + dims.nbDims, (int)1, std::multiplies<int>())
	);
	return res;
}
...
ILayer* addConv3D(
	...
	// Create plugin.
	auto plugin = plugin_factory.createConv3DPlugin(
	    conv_type,
	    kernel_dims,
	    stride_dims,
	    pad_start_dims,
...
IPlugin* StereoDnnPluginFactory::createPlugin(
