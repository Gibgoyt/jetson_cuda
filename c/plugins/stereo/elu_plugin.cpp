...
public:
	EluPlugin(DataType data_type, ILogger& log, std::string name)
	    : data_type_(data_type), format_(PluginFormat::kNCHW), max_batch_size_(0), log_(log),
	      name_(name) {
	...
	int initialize() override {
		...
		CHECK(
		    status =
		        cudnnSetActivationDescriptor(act_, CUDNN_ACTIVATION_ELU, CUDNN_PROPAGATE_NAN, 1.0)
		);
...
private:
	...
	void setTensorDescriptor() {
		...
		CHECK(cudnnSetTensorNdDescriptor(
		    t_desc_,
		    trtToCudnnDataType(data_type_),
		    tensor_dims_.nbDims,
		    tensor_dims_.d,
		    tensor_strides_.d
		));
