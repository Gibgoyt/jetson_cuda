...
cudaError_t CudaKernels::computeCostVolume(
	...
	//                                               cost_vol);
	costVolumeCopyKernel<<<g_dim, b_dim, 0, stream>>>(
	    left,
	    in_dims.d[0],
	    in_dims.d[1],
	    in_dims.d[2],
	...
	CHECKK(stream);
	costVolumeCopyPadKernel<<<g_dim, b_dim, 0, stream>>>(
	    right,
	    in_dims.d[0],
	    in_dims.d[1],
	    in_dims.d[2],
...
// FP16, NC2HW2 kernel.
__global__ void corrCostVolumeFP16NC2HW2Kernel(
    const float* left,
    const float* right,
    int32_t c,
    int32_t h,
...
template <>
cudaError_t CudaKernels::computeCorrCostVolume(
    DataType data_type,
    const float* left,
    const float* right,
    Dims in_dims,
	...
	if (data_type == DataType::kFLOAT) {
		...
		corrCostVolumeKernel<<<g_dim, b_dim, 0, stream>>>(
		    left,
		    right,
		    in_dims.d[0],
		    in_dims.d[1],
...
cudaError_t CudaKernels::addDBiasTo3DConv(
	...
	addDBiasTo3DConvKernel<<<g_dim, b_dim, 0, stream>>>(
	    bias,
	    conv_dims.d[0],
	    conv_dims.d[1],
	    conv_dims.d[2],
