...
public:
	Conv3DPlugin(
	    ...
	    : conv_type_(conv_type), w_dims_(kernel_dims), stride_dims_(stride_dims),
	      pad_start_dims_(pad_start_dims), pad_end_dims_(pad_end_dims),
	      kernel_weights_(kernel_weights), bias_weights_(bias_weights), log_(log), name_(name) {
		...
		assert(
		    pad_start_dims_.d[0] == pad_end_dims_.d[0] ||
	...
	Dims getOutputDimensions(int index, const Dims* inputs, int nbInputDims) override {
		...
		ConvUtils::setConv3DOperationDescriptors(
		    conv_type_,
		    w_dims_,
		    stride_dims_,
		    pad_start_dims_,
		...
		ConvUtils::setConv3DTensorDescriptor(
		    Conv3DType::kCuDnn,
		    y_dims_,
		    1,
		    weights_type_,
		...
		// Conv3D.
		ConvUtils::setConv3DBiasDescriptor(
		    Dims{5, {1, y_dims_.d[0], 1, 1, 1}},
		    weights_type_,
		    b_desc_,
		    log_
		);
	...
	void configure(
		...
		CHECK(cudaMemcpy(
		    kernel_weights_d_,
		    kernel_weights_.values,
		    kernel_weights_.count * elt_size,
		...
		if (bias_weights_.count > 0) {
			...
			CHECK(cudaMemcpy(
			    bias_weights_d_,
			    bias_weights_.values,
			    bias_weights_.count * elt_size,
			    cudaMemcpyHostToDevice
			));
		}
		log_.log(
	...
	int enqueue(
		...
		CHECK(
		    status = cudnnConvolutionForward(
		        ...
		        c_desc_,
		        best_algo_,
		        static_cast<uint8_t*>(workspace) + workspace_used_bytes,
		        workspace_bytes_ - workspace_used_bytes,
		        &Consts::kZero,
		        y_desc_,
		        outputs[0]
		    )
...
private:
	bool isValid() const { return cudnn_ != nullptr; }

	bool isFP16() const { return weights_type_ == CUDNN_DATA_HALF; }

	size_t getWeightsDataTypeSize() const { return (isFP16() ? sizeof(uint16_t) : sizeof(float)); }

	const void* preprocessInput(
	    int batchSize,
	    const void* x,
	    void* workspace,
	    cudaStream_t stream,
	...
	void findBestAlgo() {
		...
		auto err = cudnnFindConvolutionForwardAlgorithm(
		    cudnn_,
		    x_desc_,
		    w_desc_,
		    c_desc_,
...
// Factory method.
IPlugin* PluginContainer::createConv3DPlugin(
    Conv3DType conv_type,
    Dims kernel_dims,
    Dims stride_dims,
    Dims pad_start_dims,
	...
	plugins_.push_back(new Conv3DPlugin(
	    conv_type,
	    kernel_dims,
	    stride_dims,
	    pad_start_dims,
