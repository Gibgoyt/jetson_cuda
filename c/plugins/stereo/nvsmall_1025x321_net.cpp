...
INetworkDefinition* createNVSmall1025x321Network(
    IBuilder& builder,
	...
	auto left_conv2 = network->addConvolution(
	    *left_conv1_act->getOutput(0),
	    32,
	    DimsHW{3, 3},
	    weights.at("left_conv2_k"),
	    weights.at("left_conv2_b")
	);
	...
	auto right_conv2 = network->addConvolution(
	    *right_conv1_act->getOutput(0),
	    32,
	    DimsHW{3, 3},
	    weights.at("right_conv2_k"),
	    weights.at("right_conv2_b")
	);
	...
	auto left_conv3 = network->addConvolution(
	    *left_conv2_act->getOutput(0),
	    32,
	    DimsHW{3, 3},
	    weights.at("left_conv3_k"),
	    weights.at("left_conv3_b")
	);
	...
	auto right_conv3 = network->addConvolution(
	    *right_conv2_act->getOutput(0),
	    32,
	    DimsHW{3, 3},
	    weights.at("right_conv3_k"),
	    weights.at("right_conv3_b")
	);
	...
	auto left_conv4 = network->addConvolution(
	    *left_conv3_act->getOutput(0),
	    32,
	    DimsHW{3, 3},
	    weights.at("left_conv4_k"),
	    weights.at("left_conv4_b")
	);
	...
	auto right_conv4 = network->addConvolution(
	    *right_conv3_act->getOutput(0),
	    32,
	    DimsHW{3, 3},
	    weights.at("right_conv4_k"),
	    weights.at("right_conv4_b")
	);
	...
	auto left_conv5 = network->addConvolution(
	    *left_conv4_act->getOutput(0),
	    32,
	    DimsHW{3, 3},
	    weights.at("left_conv5_k"),
	    weights.at("left_conv5_b")
	);
	...
	auto right_conv5 = network->addConvolution(
	    *right_conv4_act->getOutput(0),
	    32,
	    DimsHW{3, 3},
	    weights.at("right_conv5_k"),
	    weights.at("right_conv5_b")
	);
	...
	auto cost_vol = addCostVolume(
	    plugin_factory,
	    *network,
	    *left_conv5->getOutput(0),
	    *right_conv5->getOutput(0),
	...
	auto conv3D_1 = addConv3D(
	    ...
	    *cost_vol->getOutput(0),
	    ...
	    Dims{3, {1, 1, 1}},
	    Dims{3, {1, 1, 1}},
	    weights.at("conv3D_1_k"),
	    weights.at("conv3D_1_b"),
	    "conv3D_1"
	);
	...
	auto conv3D_1_tran = addTransform(
	    plugin_factory,
	    *network,
	    *conv3D_1->getOutput(0),
	    {1, 0, 2, 3},
	    "conv3D_1_tran_transform"
	);
	...
	auto conv3D_2 = addConv3D(
	    plugin_factory,
	    *network,
	    *conv3D_1_act->getOutput(0),
	    Conv3DType::kTensorFlow,
	    {5, {32, 3, 32, 3, 3}},
	    Dims{3, {1, 1, 1}},
	    Dims{3, {1, 1, 1}},
	    Dims{3, {1, 1, 1}},
	    weights.at("conv3D_2_k"),
	    weights.at("conv3D_2_b"),
	    "conv3D_2"
	);
	...
	// Transpose output: KDHW -> DKHW for conv3d and DKHW -> KDHW for conv3d_transpose
	auto conv3D_2_tran = addTransform(
	    plugin_factory,
	    *network,
	    *conv3D_2->getOutput(0),
	    {1, 0, 2, 3},
	    "conv3D_2_tran_transform"
	);
	...
	auto conv3D_4 = addConv3D(
	    ...
	    *conv3D_3ds_act->getOutput(0),
	    ...
	    Dims{3, {1, 1, 1}},
	    Dims{3, {1, 1, 1}},
	    weights.at("conv3D_4_k"),
	    weights.at("conv3D_4_b"),
	    "conv3D_4"
	);
	...
	// Transpose output: KDHW -> DKHW for conv3d and DKHW -> KDHW for conv3d_transpose
	auto conv3D_4_tran = addTransform(
	    plugin_factory,
	    *network,
	    *conv3D_4->getOutput(0),
	    {1, 0, 2, 3},
	    "conv3D_4_tran_transform"
	);
	...
	auto conv3D_5 = addConv3D(
	    plugin_factory,
	    *network,
	    *conv3D_4_act->getOutput(0),
	    Conv3DType::kTensorFlow,
	    {5, {64, 3, 64, 3, 3}},
	    Dims{3, {1, 1, 1}},
	    Dims{3, {1, 1, 1}},
	    Dims{3, {1, 1, 1}},
	    weights.at("conv3D_5_k"),
	    weights.at("conv3D_5_b"),
	    "conv3D_5"
	);
	...
	// Transpose output: KDHW -> DKHW for conv3d and DKHW -> KDHW for conv3d_transpose
	auto conv3D_5_tran = addTransform(
	    plugin_factory,
	    *network,
	    *conv3D_5->getOutput(0),
	    {1, 0, 2, 3},
	    "conv3D_5_tran_transform"
	);
	...
	auto conv3D_6ds_pad = addPad(
	    plugin_factory,
	    *network,
	    *conv3D_5_act->getOutput(0),
	    {0, 0, 0, 0},
	    {1, 0, 0, 0},
	    "conv3D_6ds_pad"
	);
	...
	auto conv3D_6ds = addConv3D(
	    plugin_factory,
	    *network,
	    *conv3D_6ds_pad->getOutput(0),
	    Conv3DType::kTensorFlow,
	    {5, {128, 3, 64, 3, 3}},
	    Dims{3, {2, 2, 2}},
	    Dims{3, {0, 1, 1}},
	    Dims{3, {1, 1, 1}},
	    weights.at("conv3D_6ds_k"),
	    weights.at("conv3D_6ds_b"),
	    "conv3D_6ds"
	);
	...
	// Transpose output: KDHW -> DKHW for conv3d and DKHW -> KDHW for conv3d_transpose
	auto conv3D_6ds_tran = addTransform(
	    plugin_factory,
	    *network,
	    *conv3D_6ds->getOutput(0),
	    {1, 0, 2, 3},
	    "conv3D_6ds_tran_transform"
	);
	assert(conv3D_6ds_tran != nullptr);
	conv3D_6ds_tran->setName("conv3D_6ds_tran");

	// conv3D_6ds_act ELU activation op.
	auto conv3D_6ds_act = addElu(
	    plugin_factory,
	    *network,
	    *conv3D_6ds_tran->getOutput(0),
	    data_type,
	    "conv3D_6ds_act"
	);
	...
	auto conv3D_7 = addConv3D(
	    plugin_factory,
	    *network,
	    *conv3D_6ds_act->getOutput(0),
	    Conv3DType::kTensorFlow,
	    {5, {128, 3, 128, 3, 3}},
	    Dims{3, {1, 1, 1}},
	    Dims{3, {1, 1, 1}},
	    Dims{3, {1, 1, 1}},
	    weights.at("conv3D_7_k"),
	    weights.at("conv3D_7_b"),
	    "conv3D_7"
	);
	...
	// Transpose output: KDHW -> DKHW for conv3d and DKHW -> KDHW for conv3d_transpose
	auto conv3D_7_tran = addTransform(
	    plugin_factory,
	    *network,
	    *conv3D_7->getOutput(0),
	    {1, 0, 2, 3},
	    "conv3D_7_tran_transform"
	);
	...
	auto conv3D_8 = addConv3D(
	    plugin_factory,
	    *network,
	    *conv3D_7_act->getOutput(0),
	    Conv3DType::kTensorFlow,
	    {5, {128, 3, 128, 3, 3}},
	    Dims{3, {1, 1, 1}},
	    Dims{3, {1, 1, 1}},
	    Dims{3, {1, 1, 1}},
	    weights.at("conv3D_8_k"),
	    weights.at("conv3D_8_b"),
	    "conv3D_8"
	);
	...
	auto deconv3D_1_slice_layer = addSlice(
	    plugin_factory,
	    *network,
	    *deconv3D_1->getOutput(0),
	    ...
	    {4, {0, 0, 0, 0}},
	    {4,
	     {deconv3D_1_out_dims.d[0] - 1,
	      deconv3D_1_out_dims.d[1],
	      deconv3D_1_out_dims.d[2],
	      deconv3D_1_out_dims.d[3]}},
	    "deconv3D_1_slice"
	);
	...
	auto deconv3D_1_add_skip = network->addElementWise(
	    *(deconv3D_1_slice_layer->getOutput(0)),
	    *(conv3D_5_act->getOutput(0)),
	...
	auto deconv3D_2_slice_layer = addSlice(
	    plugin_factory,
	    *network,
	    *deconv3D_2->getOutput(0),
	    ...
	    {4, {0, 0, 0, 0}},
	    {4,
	     {deconv3D_2_out_dims.d[0] - 1,
	      deconv3D_2_out_dims.d[1],
	      deconv3D_2_out_dims.d[2],
	      deconv3D_2_out_dims.d[3]}},
	    "deconv3D_2_slice"
	);
	...
	auto deconv3D_2_add_skip = network->addElementWise(
	    *(deconv3D_2_slice_layer->getOutput(0)),
	    *(conv3D_2_act->getOutput(0)),
	    ElementWiseOperation::kSUM
	);
