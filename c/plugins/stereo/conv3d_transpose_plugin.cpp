	...
	int enqueue(
		...
		if (bias_weights_.count > 0) {
			...
			if (b_dims_.d[1] == x_dims_.d[0])
				...
				CHECK(
				    cu_status = CudaKernels::addDBiasTo3DConv(
				        bias_weights_d_,
				        b_dims_,
...
// Factory method.
IPlugin* PluginContainer::createConv3DTransposePlugin(
    Conv3DType conv_type,
    Dims kernel_dims,
    Dims out_dims,
    Dims stride_dims,
	...
	plugins_.push_back(new Conv3DTransposePlugin(
	    conv_type,
	    kernel_dims,
	    out_dims,
	    stride_dims,
