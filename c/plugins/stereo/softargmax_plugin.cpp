...
public:
	...
	int enqueue(
		...
		CHECK(
		    cu_status =
		        cudaMemcpyAsync(pdst, inputs[0], in_size_bytes, cudaMemcpyDeviceToDevice, stream)
		);
	...
	size_t getSerializationSize() override { return serialize().size(); }
...
// Factory method.
IPlugin* PluginContainer::createSoftargmaxPlugin(
    DataType data_type,
