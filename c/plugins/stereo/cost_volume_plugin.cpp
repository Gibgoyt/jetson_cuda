...
public:
	CostVolumePlugin(
	    ...
	    : data_type_(data_type), format_(PluginFormat::kNCHW), cv_type_(cv_type),
	...
	int enqueue(
		...
		if (cv_type_ == CostVolumeType::kDefault)
			CHECK(
			    status = CudaKernels::computeCostVolume(
			        data_type_,
			        pleft,
			        pright,
			        in_dims_,
			...
			CHECK(
			    status = CudaKernels::computeCorrCostVolume(
			        data_type_,
			        pleft,
			        pright,
			        in_dims_,
...
// Factory method.
IPlugin* PluginContainer::createCostVolumePlugin(
    DataType data_type,
    CostVolumeType cv_type,
    int max_disparity,
    std::string name
