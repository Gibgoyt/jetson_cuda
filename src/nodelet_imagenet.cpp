...
public:
	...
	void onInit() {
		...
		if (access(prototxt_path.c_str(), R_OK))
			ROS_ERROR(
			    "unable to read file \"%s\", check filename and permissions",
			    prototxt_path.c_str()
			);
...
private:
	void callback(const sensor_msgs::ImageConstPtr& input) {
		...
		// classify image
		const int img_class =
		    net->Classify(imgCvt->ImageGPU(), imgCvt->GetWidth(), imgCvt->GetHeight(), &confidence);
	...
	PLUGINLIB_DECLARE_CLASS(
	    ros_deep_learning,
	    ros_imagenet,
	    ros_deep_learning::ros_imagenet,
	    nodelet::Nodelet
	);
#endif
